import {ADD_RECENT_CONTACT, CHANGE_PASSWORD, CHANGE_PHONE_NUMBER, UPDATE_PROFILE, VERIFY_EMAIL,} from './constants';
  ...
  switch (action.type) {
    ...
    case ADD_RECENT_CONTACT:
      if (state.recentContacts.findIndex(card => card.cardId === action.payload.cardId) >= 0)
        return {
          ...state,
          recentContacts: [action.payload, ...state.recentContacts.filter(card => card.cardId !== action.payload.cardId).slice(0, 4)]
        }
      else
